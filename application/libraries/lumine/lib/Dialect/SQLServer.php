<?php
/**
 * Classe para dialeto com o banco SQL Server
 ...
 * @link http://www.hufersil.com.br
 */
class Lumine_Dialect_SQLServer extends Lumine_EventListener implements Lumine_Dialect_IDialect
{
		...
		try
		{
			...
			switch($mode)
			{
				case Lumine_Base::FETCH_ROW:
					$native_mode = SQLSRV_FETCH_NUMERIC;
				break;
				
				case Lumine_Base::FETCH_BOTH:
					$native_mode = SQLSRV_FETCH_BOTH;
				break;
				
				case Lumine_Base::FETCH_ASSOC:
				default:
					$native_mode = SQLSRV_FETCH_ASSOC;
			}
	...
	public function fetch_row($rowNumber)
	{
		...
		$row = sqlsrv_fetch_array($this->resultList[$this->getObjectId()], SQLSRV_FETCH_ASSOC, SQLSRV_SCROLL_ABSOLUTE, $rowNumber);
	...
	public function fetch()
	{
		...
		$row = sqlsrv_fetch_array( $this->resultList[$this->getObjectId()], SQLSRV_FETCH_ASSOC, SQLSRV_SCROLL_ABSOLUTE, $pointer);
		$this->pointerList[$this->getObjectId()]++;
	...
	{
		$cn = $this->getConnection();
		$rs = $cn->executeSQL('SELECT @@IDENTITY');
	...
	public function freeResult($resultID){
		if( isset($this->resultList[ $resultID ]) && is_resource($this->resultList[ $resultID ]) ){
			...
			sqlsrv_free_stmt($this->resultList[ $resultID ]);
		}
	}
