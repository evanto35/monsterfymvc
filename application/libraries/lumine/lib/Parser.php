...
class Lumine_Parser {
	 ...
	 * @return string
	 */
	public static function parsePart(Lumine_Base $obj, $str, $args = null, array $extraEntities = array())
	{
		$total = strlen($str);
		$inStr = false;
		$start = '';
		...
		for($i=0; $i<$total; $i++) 
		{
			...
			if($inStr == false && ($c == "'" || $c == '"'))
			{
				$tmp = self::parseEntityValues($obj, $tmp, $args, $extraEntities);
				$nova .= $tmp;
				$tmp = '';
				$inStr = true;
				$start = $c;
		...
		if($tmp != '')
		{
			...
			$nova .= self::parseEntityValues($obj, $tmp, $args, $extraEntities);
		}
	...
	public static function parseEntityValues(Lumine_Base $obj, $str, $args = null, array $extraEntities = array()){
		...
		$list = array_merge($extraEntities, $obj->_getObjectPart('_join_list'));
			...
			if( !is_array($val) ){
				switch($wildcards[$i][0]){
					...
					default:
						...
						// 17/01/2012 - colocando um delimitador para indicar que deve ser uma palavra
						// inteira
						$part = substr($str, $lastOffset, $wildcards[$i][1] - $lastOffset);
						if(preg_match('@\b(i)?like\b@i', $part)){
