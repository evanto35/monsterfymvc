...
class Lumine_Base extends Lumine_EventListener implements Iterator {
    ...
    public function join(Lumine_Base $obj, $type = 'INNER', $alias = '', $linkName = null, $linkTo = null, $extraCondition = null) {
        ...
        if (!is_null($extraCondition)) {          // se a pessoa definiu uma condicao extra
            ...
            try {
                $extraCondition = Lumine_Parser::parsePart($obj, $extraCondition, $args, array($this)); // faz o parser para certificacao que os campos existem certinho
                $extraCondition = Lumine_Parser::parseEntityNames($obj, $extraCondition, array($this));
            } catch (Exception $e) {

                try {
                    $extraCondition = Lumine_Parser::parsePart($this, $extraCondition, $args, array($obj)); // faz o parser para certificacao que os campos existem certinho
                    $extraCondition = Lumine_Parser::parseEntityNames($this, $extraCondition, array($obj));
                } catch (Exception $e) {
    ...
    public function joinExpression($obj, $type, $expression, $alias = null) {
        ...
        try {
            $expression = Lumine_Parser::parsePart($obj, $expression, $args, array($this)); // faz o parser para certificacao que os campos existem certinho
            $expression = Lumine_Parser::parseEntityNames($obj, $expression, array($this));
        } catch (Exception $e) {

            try {
                $expression = Lumine_Parser::parsePart($this, $expression, $args, array($obj)); // faz o parser para certificacao que os campos existem certinho
                $expression = Lumine_Parser::parseEntityNames($this, $expression, array($obj));
            } catch (Exception $e) {
                Lumine_Log::warning('Houve um erro na analise da condicao extra');
            }
        }
    ...
    public function formattedValue($member) {
        ...
        if (isset($this->_formatters[$member])) {
            $this->dispatchEvent(new Lumine_Events_FormatEvent(Lumine_Event::PRE_FORMAT, $this, $member, $oldvalue));

            foreach ($this->_formatters[$member] as $formatter) {
                $newvalue = call_user_func_array($formatter, array($newvalue));
            }

            $this->dispatchEvent(new Lumine_Events_FormatEvent(Lumine_Event::POS_FORMAT, $this, $member, $oldvalue, $newvalue));
        }
