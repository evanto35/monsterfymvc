...
class Alert {
    ...
    private static function echoAlert(Alert $Alert) {
        echo "<div style='height: 20px;' id='{$Alert->id}' class='alert alert" 
             . $Alert->type 
             . " fade in'>"
             . $Alert->text 
             . ($Alert->showIcon ? "<button class='close' data-dismiss='alert' href='#'>&times;</a>" : "")
             . "</div>";
    }

    public static function echo($unset = true) {
        if (isset($_SESSION[self::SESSION_INDEX])) {

            foreach($_SESSION[self::SESSION_INDEX] as $alert) {
                self::echoAlert(unserialize($alert));
            }       
        }
